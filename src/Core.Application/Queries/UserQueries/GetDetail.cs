...
namespace SwanseaCompSci.LabManagementSystem.Core.Application.Queries.UserQueries
{
    ...
    public partial class GetDetail
    {
        ...
        {
            public Response(UserDetailModel? resource)
            {
                Resource = resource;
            }

            public UserDetailModel? Resource { get; set; } = null!;
        }
        ...
        internal sealed class QueryHandler : IRequestHandler<Query, Response>
        {
            ...
            {
                var specification = new GetUserDetailSpecification(userId: request.UserId);

                var user = await Repository.GetItemAsync(specification: specification,
                                                         cancellationToken: cancellationToken);

                return user is not null
                    ? new Response(resource: Mapper.Map<User, UserDetailModel>(user))
