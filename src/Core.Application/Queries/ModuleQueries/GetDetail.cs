...
namespace SwanseaCompSci.LabManagementSystem.Core.Application.Queries.ModuleQueries
{
    ...
    public sealed class GetDetail
    {
        ...
        {
            public Response(ModuleDetailModel? resource)
            {
                Resource = resource;
            }

            public ModuleDetailModel? Resource { get; set; } = null!;
        }
        ...
        internal sealed class QueryHandler : IRequestHandler<Query, Response>
        {
            ...
            public async Task<Response> Handle(Query request, CancellationToken cancellationToken)
            {
                ISingleResultSpecification<Module> specification = CurrentUserService.UserApplicationRole == ApplicationRole.Administrator
                    ? new GetModuleDetailSpecification(moduleId: request.ModuleId)
                    : new GetModuleDetailWherePermissionSpecification(moduleId: request.ModuleId,
                ...
                return module is not null
                    ? new Response(resource: Mapper.Map<Module, ModuleDetailModel>(module))
