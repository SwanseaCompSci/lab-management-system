    ...
    public sealed class Create
    {
        ...
        internal sealed class CommandHandler : IRequestHandler<Command, Response>
        {
            ...
            public async Task<Response> Handle(Command request, CancellationToken cancellationToken)
            {
                ...
                var labSchedule = await Repository.AddItemAsync(item: new LabSchedule(labId: request.LabId,
                                                                                      start: startDateTime,
                                                                                      end: endDateTime),
                                                                cancellationToken: cancellationToken);

                var labScheduleModel = Mapper.Map<LabSchedule, LabScheduleModel>(labSchedule);

                return new Response(resource: labScheduleModel);
            }
        }
    }
}
