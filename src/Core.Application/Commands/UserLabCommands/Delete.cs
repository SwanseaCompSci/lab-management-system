...
namespace SwanseaCompSci.LabManagementSystem.Core.Application.Commands.UserLabCommands
{
    ...
    public sealed class Delete
    {
        ...
        internal sealed class CommandHandler : IRequestHandler<Command, Response>
        {
            ...
            {
                var userLab = await Repository.DeleteItemAsync(userId: request.UserId,
                                                               labId: request.LabId,
                                                               cancellationToken: cancellationToken);

                return userLab is not null
                    ? new Response(resource: Mapper.Map<UserLab, UserLabModel>(userLab))
