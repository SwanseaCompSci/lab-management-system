...
{
    public sealed class TestsUserRemovedFromLabDomainEventHandler : TestBase
    {
        [Test]
        public async Task Handle_Command_Success()
        {
            ...
            Testing.RunOnDateTime(dateTime: new DateTime(2023, 02, 14));
            ...
            var domainEvent = new UserRemovedFromLabDomainEvent(userId: user.Id, labId: lab.Id);
            var notification = new DomainEventNotification<UserRemovedFromLabDomainEvent>(domainEvent: domainEvent);
            ...
            applicationDbContext.UserLabSchedules.Should().HaveCount(2);
            applicationDbContext.UserLabSchedules
                .Include(x => x.LabSchedule)
                .Any(x => x.UserId == user.Id && x.LabSchedule.Start > new DateTime(2023, 02, 14))
                .Should().BeFalse();
            ...
            resultTimeAvailability.IsAllocated.Should().BeFalse();
        }
    }
}
