...
{
    public sealed class TestsGenerateQuestionnaireTokenForAllCommandHandler : TestBase
    {
        [Test]
        public async Task Handle_Command_Success()
        {
            ...
            var command = new GenerateQuestionnaireTokenForAll.Command();
            ...
            {
                response.Resource.Any(x => x.Id == item.Id && x.QuestionnaireToken is not null).Should().BeTrue();
            }
        }

        [Test]
        public async Task Handle_Command_No_Resources_To_Update()
        {
            // Arrange
            Testing.RunAsUser(user: Users.GetDefaultUser());
